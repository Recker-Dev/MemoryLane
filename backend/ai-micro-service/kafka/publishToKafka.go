...
func (ph *PublisherHandler) SendMessage(topic, key string, value []byte) error {
	...
	_, _, err := ph.publisher.SendMessage(msg)
	if err != nil {
		log.Printf("[Kafka Publisher] Kafka send error: %v", err)
		return err
	}
	// log.Printf("[Kafka Publisher] Sent to Kafka topic=%s partition=%d offset=%d", topic, partition, offset)
	return nil
}
